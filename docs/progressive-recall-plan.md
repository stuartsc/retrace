...
**Status: proposed; implementation remains on hold for Stuart's review.** This revision incorporates the checkpoint review and the Timely-informed activity/context recommendations. Saving or committing the plan does not implement it, install a build or establish a release. Existing implemented fixes and local trials remain documented in [CHANGELOG.md](../CHANGELOG.md).
