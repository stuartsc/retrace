...
**Phase 2C is implemented, passes optimized compilation and the complete 989-test suite, with five intentional skips and zero failures.** It adds durable native evidence publication, bounded bootstrap/replay and separate unready lexical/vector bookkeeping. It has not been installed. Hybrid retrieval in 2D remains next; see the [implementation and validation record](progressive-recall-validation.md#native-screen-feed-and-bootstrap--2026-09-15).
...
The initial integration audit found that the primary UI route uses `DataAdapter.search`, while `SearchManager`/`ResultRanker` are a separate lexical fallback. `SearchViewModel` currently truncates at 15 words; remove that limit before evaluating full-question retrieval. The excluded vector code also truncates embedding inputs, drops scoped filters/cursors and scans vectors by bare frame ID. Keep it excluded. Expand the two reviewed Cedar screenshots with independently reviewed authored distractors and fixed questions before comparing lexical, embedding-only and combined ranking. Freeze the model artifact/hash, tokenizer and preprocessing, engine revision, thread/backend configuration and ranking parameters; the old sketch is not a verified pin. Report exact-reference ranking, amounts/negations, source/deletion refusal and resource measurements separately from user acceptance.
