...
**Priority update — September 20:** Stuart reported character errors and requested stronger screen/window/text capture, direct native text where available, and high-quality originals retained through OCR before efficient archival compression. The [capture/text quality implementation and delivery plan](capture-text-quality-plan.md) now precedes semantic index/worker integration. The first source checkpoint keeps accurate OCR at native resolution with bounded overlapping crops and removes historical OCR's intermediate JPEG conversion. Authored native/codec comparisons expose both accuracy gains and increased processing cost. Original retention, direct visible app text and representative performance/compression acceptance remain pending. This checkpoint is not installed.

**Current local trial: 0.7.6 (2609.18.1), installed September 18.** This includes the completed Phase 2 checkpoints and timeline transcript play/pause controls. The reviewed source passes **1,078 Swift tests, five intentional skips, zero failures**, and optimized compilation. Retrace launched hidden; live read-only checks verified V23, unchanged store identity/settings and fresh completed captures. Audible playback, wider installed UI checks and recall/resource acceptance remain separate; see the [installation evidence](progressive-recall-validation.md#local-trial-2609181--2026-09-18).

**Phase 2D's native admission checkpoint passed its then-complete 1,067-test Swift suite, with five intentional skips and zero failures, and optimized compilation.** Writer-owned policy/source fences, bounded execution claims and unpublished artifact receipts are implemented, independently reviewed and included in the current local trial. Both index channels remain unready; bounded index/worker integration is the next gate. See the [admission validation record and limits](progressive-recall-validation.md#native-writer-admission-and-unpublished-artifacts--2026-09-17).

The earlier question-preservation and authored-comparison checkpoint passed 1,004 Swift tests, eleven Python benchmark regressions and optimized compilation. Complete questions survive submission, recent replay and pagination. The frozen comparison supports MiniLM plus the existing exact lexical channel as the next integration candidate; a known ambiguous oracle is excluded transparently. See the [comparison and measurement limits](progressive-recall-validation.md#full-questions-and-authored-retrieval-comparison--2026-09-16).

**Phase 2C is included in the current local trial.** Its durable native evidence publication, bounded bootstrap/replay and separate unready lexical/vector bookkeeping passed optimized compilation and the then-complete 989-test suite. See the [Phase 2C implementation and validation record](progressive-recall-validation.md#native-screen-feed-and-bootstrap--2026-09-15).

**Phase 2A/2B first entered local assessment as 2609.15.1 and remains included.** Settings and exact timeline navigation are repaired in source, the project/visit surface is removed, and Screenshots shares the structured OCR/evidence inspector. The independently verified package was installed and launched hidden on September 15. The Settings status-menu route opened its native window, and all 31 captures in that bounded post-launch cohort completed. Full timeline/Screenshots visual checks, resource limits and broader recall acceptance remain open; see the [trial evidence](progressive-recall-validation.md). Stuart's instruction to commence Phase 2 supersedes the previous hold on later work; it does not retroactively pass those gates.
...
**2C — durable evidence publication and bootstrap (installed local trial):** transactional revision/tombstone feed entries, a coherent bootstrap boundary, applied-event/work persistence and independent lexical/vector readiness bookkeeping. `recall_search_revision` is an invalidation stamp, not a change feed. Multiple context observations sharing retained pixels need a separate schema/API design; the current schema retains one screen observation per store/frame.

The initial 2C contract covers **already-materialized native screen observations**, not the full legacy library or externally mutable imported stores. V22 adds ordinary-table publication triggers to the existing canonical transactions, so a retained V21 writer cannot silently bypass publication. Events contain opaque, source-qualified references and state changes; OCR and context stay in the immutable extraction payload. A stable feed identity and monotonic sequence identify each event. Per-observation source-state sequences also distinguish same-revision media/redaction changes.

Bootstrap records the feed boundary and maximum materialized native frame ID, then uses short keyset pages over that fixed range. Pages observe current state; replay after the recorded boundary reconciles intervening changes. This is scan plus replay, not a historical SQLite snapshot. The database chooses and applies each bounded page, writing applied-event identities, separate lexical/vector work and the checkpoint in one transaction. Clients cannot acknowledge arbitrary sequences. Leases bind feed, native store, consumer and bootstrap generation; bounded compaction retains required history and expired/gapped cursors require rebootstrap. At most 32 consumer IDs are registered, with expired IDs reusable; current work has two rows per consumer/observation, keeping publication invalidation bounded rather than accumulating historical lease rows.

The durable work records created in 2C contain only identity/state and remain **blocked, invalidated or deleted**. There is no model, worker, copied-text staging, automatic consumer loop or result-acceptance API. Lexical and vector readiness remain separately unready; a consumed event does not mean indexed evidence. Before 2D can accept derived results, introduce a durable writer-owned policy/source fence covering both capture configuration assignment paths (startup and runtime updates). A read-time configuration check or notification is not an atomic acceptance fence. Current exact expansion/resolution continues to enforce current disclosure policy independently.

Implementation coordination: root owns the Shared feed models/protocol, App entry points and documentation; `recovery_review` owns the additive migration/publication triggers and publication regressions; `quit_review` owns consumer/bootstrap/compaction persistence and its regressions; `notification_fix` independently reviews the contract and implementation. Tests use authored SQLite/filesystem fixtures. All Swift edits pause during root-run SwiftPM validation. No live-library migration or installation is implied by this implementation slice.

Root also owns the debug-only file-logging isolation in `Shared/Logging.swift` and its App filesystem regressions. A paired native-helper probe found that the inherited validation sandbox removed bundle metadata needed by existing identity tests. Process-launch logging admission lets normal native tests run without persisting diagnostics to the installed app's files; production logging and identity requirements stay intact.
...
The September 16 authored comparison supports **MiniLM plus exact lexical retrieval as the next integration candidate**: 11/11 first results on the unambiguous subset, versus 4/11 for the current full-question lexical parser. BGE adds no accuracy on that subset and substantial latency, so keep it out of the default interactive path until a harder corpus demonstrates benefit. Q11's ambiguous oracle is excluded transparently; original raw data remain retained. This small experiment does not establish broad retrieval quality or production-scale speed, and its 19.333-second worker-entry initialization does not fit the ten-second Low Power Mode unit allowance. See [the measured result and limits](progressive-recall-validation.md#measured-result-and-next-model-choice).

The primary UI route uses `DataAdapter.search`, while `SearchManager`/`ResultRanker` are a separate lexical fallback. The first 2D change preserves full questions, exclusion suffixes, recent-entry representation and the committed query during pagination. The excluded vector code still truncates embedding inputs, drops scoped filters/cursors and scans vectors by bare frame ID; keep it excluded.

Deliver 2D through three explicit gates:

1. **Question preservation and authored comparison — implemented and measured September 16.** The eight independently reviewed JPEGs and twelve frozen questions are in `docs/fixtures/progressive-recall/phase2d/`; interpretation excludes the documented ambiguous Q11. `ProgressiveRecallBenchmarkTests` binds real Vision/SQLite results to exact references through the primary route. The offline evaluator in `scripts/recall_benchmark/` compares full-question lexical, separately labelled authored keyword queries, MiniLM, fusion and the full-pool BGE reranker on the same permitted candidates. It pins the exact export SHA as well as model artifacts, complete loader inventory, tokenizer/preprocessing, runtime versions, backend/threads and ranking parameters. Exact-reference ranking, amounts/negations, refusal checks and resource costs are reported separately from user acceptance. This test-only evaluator grants no production index readiness.
2. **Durable writer-owned admission — implemented and reviewed September 17.** V23 adds a native-only policy/session fence and separate claim/result receipts. Startup and runtime `CaptureManager` configuration assignments first durably deactivate the old epoch, apply the configuration serially, then activate that exact new epoch. This boundary initializes after the database opens and before any derived read/claim. Failure/cancellation leaves admission inactive; A→B→A cannot revive an old claim. Each startup creates a fresh capable writer incarnation. Deadline checks resample after SQLite waits and canonical validation; closing prevents replacement authority throughout checkpoint retry. The 63 new regressions pass in the complete 1,067-test suite. This gate stores unpublished artifacts and grants no model scheduling or index readiness.
3. **Bounded production integration.** Only after the measured model choice and admission gate, build the local index/worker and connect Find/Score to the primary route. Preserve restrictions before limits, complete exact references, independent channel coverage and explicit continuations. Evaluate larger pools, cancellation, foreground impact and actual-user questions before enabling automatic history work.

The admission claim binds the native feed/store, complete observation/revision/block scope, source-state sequence, consumer lease, policy epoch/writer incarnation, bounded worker attempt/deadline and transformation/input fingerprint. The consumer lease is not a worker execution lease. Accept in one synchronous writer transaction only after reloading every fence, checking current retained metadata/policy, deletion/redaction/media state and validating the output. Identical retries return the existing receipt; conflicting duplicates fail. Partial chunks cannot mark a whole observation ready. Retrieval joins current policy/source fences instead of trusting a stored readiness flag, allowing constant-size policy revocation without scanning historical work.

V22 triggers cover source mutations from retained older writers, but an old binary cannot publish new configuration policy epochs. Require a capable owning coordinator and fresh activation on every startup; concurrent older policy ownership needs a separate compatibility design. Imported derived work and agent disclosure remain denied initially. Required real SQLite regressions include policy/source A→B→A, startup/runtime failure/cancellation, stale/replaced leases, delayed OCR/deletion/redaction/media changes through retained writer shapes, transaction/COMMIT rollback, idempotency/conflicting outputs, partial chunk coverage and constant-size revocation with large unrelated work sets.

Gate 2 coordination: root owns Shared admission contracts, App integration and documentation; `recovery_review` owns V23 and synchronous admission persistence; `notification_fix` owns Capture's serialized configuration boundary; `quit_review` independently reviews the contracts, implementation and regressions. All Swift and fixture inputs freeze during root-owned validation.

The gate-2 result boundary stores **unpublished opaque artifacts**, with an exact input-page digest and an atomic checksum/receipt. It is not a typed index publication API: lexical/vector work stays unready, and workers receive no input text through this claim API yet. Gate 3 must supply the bound worker input, validate the chosen lexical/vector representation and publish that index transactionally. Use at most 256 retained attempts/receipts, 256 KiB per artifact and 16 MiB of artifact bytes per native store; cleanup handles at most 100 rows per call. Execution attempts last at most 30 seconds, with both wall-clock and same-incarnation monotonic deadlines. Completed receipts may survive that execution deadline for up to 24 hours, but every read/retry still needs the exact current policy, source and consumer fences.

The source audit extends V22 with a constant-size native metadata epoch for frame-to-segment relinking and app/title/URL changes, including changes that return to their original values. This conservative epoch may invalidate unrelated held work. It does not advance for routine new capture/FTS insertion or initial OCR/dimension establishment. Narrow persistent guards preserve native retained payload immutability and monotonic preferred revisions while allowing supported tombstone deletion and dimension establishment. Explicit evidence deletion also removes its staged bytes in that transaction. A new database actor cannot recover authority from a persisted active bit; supported activation always requires a fresh live owning capability.
