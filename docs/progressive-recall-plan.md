...
**Priority update — September 20:** Stuart reported character errors and requested stronger screen/window/text capture, direct native text where available, and high-quality originals retained through OCR before efficient archival compression. The [capture/text quality audit and delivery plan](capture-text-quality-plan.md) now precedes semantic index/worker integration. It identifies actual 4K→1.75 MP OCR reduction, pre-OCR original disposal and a JPEG conversion in historical OCR, with an authored native probe. These are investigated mechanisms and planned changes, not an installed correction.
