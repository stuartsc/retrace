...
**Phase 2D's question-preservation and authored-comparison checkpoint passes the complete 1,004-test Swift suite, with five intentional skips and zero failures, eleven Python benchmark regressions and optimized compilation.** Complete questions now survive submission, recent replay and pagination. The frozen comparison supports MiniLM plus the existing exact lexical channel as the next integration candidate; a known ambiguous oracle is excluded transparently. Production hybrid retrieval still needs durable writer-owned admission and bounded integration. This checkpoint has not been installed; see the [validation record and measurement limits](progressive-recall-validation.md#full-questions-and-authored-retrieval-comparison--2026-09-16).
...
2. **Durable writer-owned admission.** Before accepting derived results, add a native-only policy/session fence and separate claim/result receipts. Startup and runtime `CaptureManager` configuration assignments must first durably deactivate the old epoch, apply the configuration serially, then activate that exact new epoch. Initialize this boundary after the database opens and before any derived read/claim. Failure/cancellation leaves admission inactive; A→B→A cannot revive an old claim. Each startup creates a fresh capable writer incarnation. Current read-time checks and post-update coordinator notifications are insufficient.
