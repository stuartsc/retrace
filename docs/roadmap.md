...
**Status:** progressive-recall Phase 2 is authorised. The current recorded local trial is **2609.18.1**, including evidence navigation, structured observations, native feed/bootstrap, complete questions, native admission and transcript playback. Live V23 and resumed recording were verified September 18. Admission stores bounded unpublished artifacts; both derived search channels remain unready. Stuart's September 20 [capture/text quality priority](capture-text-quality-plan.md) now precedes index/worker integration. Native-resolution crop OCR and direct archive-pixel extraction are implemented in an uninstalled source checkpoint; original retention, direct app text and representative archival/performance comparisons remain pending. The [validation ledger](progressive-recall-validation.md) distinguishes installed behavior from acceptance. The complete multi-surface and semantic representation below is not yet implemented. The numbered product phases later in this roadmap are distinct from the progressive-recall implementation phases.
